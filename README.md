...
`rl_uploader.exe` と同じフォルダに、ファイル名 `.env`(ドット始まり・拡張子なし)のテキストファイルを作ります。

> **メモ帳で `.env` を作るコツ**:
> 「名前を付けて保存」で、ファイル名欄に `".env"`(**ダブルクォート付き**)と入力、「ファイルの種類」を「**すべてのファイル**」にして保存すると、`.env.txt` にならず正しく `.env` になります。

中身を以下のようにして保存:

```
BALLCHASING_TOKEN=ここに手順1でコピーしたTokenを貼り付け
```
...
- 同じフォルダに `.env` ファイルがない、または `BALLCHASING_TOKEN=` の行がない
- → 手順 4 を見直してください
- メモ帳で保存したら `.env.txt` になっていた可能性も(エクスプローラーで「拡張子を表示」して確認)
...
4. Create a `.env` next to `rl_uploader.exe` containing `BALLCHASING_TOKEN=<your-token>`.
