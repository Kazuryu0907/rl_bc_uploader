...
1. エクスプローラーで Rocket League のインストール先の `TAGame\Config\` フォルダを開く
   - **Steam 版**: `C:\Program Files (x86)\Steam\steamapps\common\rocketleague\TAGame\Config\`(デフォルト)
     - 場所が分からない場合: Steam で Rocket League を右クリック → 「管理」 → 「ローカルファイルを参照」 で開いたフォルダの `TAGame\Config\`
...
4. もし `Port=` の行があれば `49123` のままにしてください(変えると `.env` の `RLS_TCP_ADDR` も合わせる必要があります)

> ⚠️ `PacketSendRate=0` のままだとツールがデータを受け取れません。必ず `30` などの正の数にしてください。
> ⚠️ Rocket League のアップデート後にこのファイルが初期化されることがあります。突然動かなくなったらもう一度ここを確認してください。
...
> **「Windows によって PC が保護されました」**(SmartScreen)が出ることがあります。署名なし配布のため避けられません。「詳細情報」→「実行」で起動できます。不安なら[ソースから自分でビルド](#ソースからビルド)してください。
...
> **無料アカウントは 1 日 10 アップロードまで**です。超過すると `[失敗] アップロード: HTTP 429` が出てアップロードされません(翌日リセット)。

`rl_uploader.exe` のウィンドウは Rocket League をプレイ中はずっと開いたままにしておいてください。閉じると自動アップロードも止まります。Rocket League を後から起動・再起動した場合は自動で再接続するので、ツール側を再起動する必要はありません。
...
### `[失敗] アップロード: HTTP 429` のメッセージが出る
- ballchasing.com の無料枠(1 日 10 アップロード)を超えた
- → 翌日リセットされるので待つ、または ballchasing の有料プランを検討

### Rocket League のアップデート後に動かなくなった
- ゲーム本体のアップデートで `DefaultStatsAPI.ini` が初期値(`PacketSendRate=0`)に戻ることがあります
- → 手順 2 をやり直してから Rocket League を再起動してください
